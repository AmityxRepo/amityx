...
## D-015: Guardian media access via a signing Edge Function, not broad Storage RLS — 2026-07-13
Choice: Signed URLs for parent-facing photos are minted by a deployed Supabase Edge Function (`guardian-media`) that re-validates the guardian_link token server-side before signing — the private Storage bucket has no anon SELECT policy at all. Photo-consent is enforced at WRITE (a photo tagging any non-consented child is rejected outright, naming the blocker) rather than filtered at read. The 30-day purge runs via a GitHub Actions cron + script (not `pg_cron`), because pure SQL deletion would orphan Storage bytes — the script removes bytes and rows together.
Because: T-011's developer found that a Storage policy broad enough to let `anon` sign a known object path also permits bucket enumeration via `list()` — unacceptable for child media. Reject-at-write keeps the read-side invariant ("a distributed photo has zero non-consented subjects") true by construction, with no runtime filtering logic that could have a bug.
Affects: T-011, `supabase/functions/guardian-media/`, criterion 4; the R2 stage-2 flip (D-011) must preserve this signing pattern, not fall back to a public/broad-read bucket.
